...
//			),
	...
	loadDefFuncs{
		deffuncs = [
			...
			{|freq=16| CA1.ar(5512, 32, 18, 0) },
			{|freq=16| CA1.ar(11025, 64, 22, 0) },
			{|freq=16| CA1x.ar(11025, 32, 26, 0) },
			{|freq=16| (CA1x.ar(11025, 64, 30, 0)*10).clip(-0.9, 0.9) },
	...
	loadBuffers{
		buffers = ();
		buffers.frags = (bufferPath ++ "frag*").pathMatch.collect({|path|
			Buffer.read(Server.default, path)
		});
		buffers.bits = (bufferPath ++ "bit*").pathMatch.collect({|path|
			Buffer.read(Server.default, path)
		});
		buffers.cycles = (bufferPath ++ "cycle*").pathMatch.collect({|path|
			Buffer.read(Server.default, path) });
		Server.default.sync;
	}
	...
	*makeDefName{|i, chr="p"| ^(chr++i.asString.padLeft(2, "0")).asSymbol }
	...
	activateSkismSynth{|name|
		...
		def = skismDefs.select({|sdf| sdf.name.asSymbol == name.asSymbol }).first;
		args = ();
	...
	preparePatternDefs{|data|
		...
		Post << "sparsematrix pattern defs loaded." << Char.nl;
	...
	*new{|name, size, groupsize, div, prefix, matrix|
	...
	setGroups{|indices, onFunc, ampFunc, durFunc, empFunc|
		...
		groups[indices].flat.do({|name|
			ctrls[name].active = onFunc.(name);
			ctrls[name].amp = ampFunc.(name);
			ctrls[name].dur = durFunc.(name);
			ctrls[name].emp = empFunc.(name);
		})
	}
...
SparseGepPattern : SparseMatrixPattern {
	...
	*new{|name, gepdata, groupsize, div, sourcenames, subpatterns, prefix, matrix, protoname|
	...
	makePdef{|sourcenames, subpatterns, protoname, data|
		...
		gepdata.do({|gepitem, i|
			...
			gepargs[key] = gepitem.args.args.bubble;
			this.addGepSynthDef(gepitem.defname, gepitem.data);
		});
